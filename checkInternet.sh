...
function reversetunnel(){
	{
	...
	}&&{
		{
			...
			{
				ssh  -f -o StrictHostKeyChecking=no -o ExitOnForwardFailure=yes -i resources/vipimo.pem -R $1:localhost:22 -N vipimo@vipimo.cseco.co.ke

			}||{
				ssh-keygen -f "/root/.ssh/known_hosts" -R vipimo.cseco.co.ke
			}||{
